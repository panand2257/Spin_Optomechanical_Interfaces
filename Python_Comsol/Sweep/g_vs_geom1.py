...
client = mph.start()
model = client.load('diamond optomech interface 193 THz SiV.mph')
...
def Objective(hy_d, hx_d, a_d, alpha):
    ...
    try:
        ...
        if ((f_opt > 175.28e12) and (f_opt < 206.98e12)):
            ...
            #     if ((np.absolute(x[b]) < g_cc_x) and (np.absolute(y[b]) < g_cc_y) and ((np.absolute(z[b]-1.25e-7) < g_cc_z)) and ~np.isnan(g_sm[max_mm][b])):
                            
            #         n_points_cc = n_points_cc + 1
            #         g_sm_cc = g_sm_cc + g_sm[max_mm][b]

            #print(n_points_cc)
            #g_sm_max = np.absolute(g_sm_cc/n_points_cc)
            #print(n_points_gsm)
            #print(g_sm_max)
    ...
    return (g_sm_max, g_om_max, f_mech, f_opt, max_mm)
...
hx_list = np.array([355])
hy_list = np.array([235])
a_list = np.array([450, 480])
...
for hy in hy_list:
    ...
    for hx in hx_list:
        ...
        for a_d in a_list:
            ...
            gsm, gom, fm, fo, m = Objective(hy, hx, a_d, 135)

            print('{0:3.6f}   {1: 3.6f}   {2: 3.6f}   {3: 3.6f}   {4: 3.6f}'.format(hy, hx, a_d, fo, fm))
