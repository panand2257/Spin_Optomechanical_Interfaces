...
client = mph.start()
model = client.load('Si optomech interface 193 THz-test.mph')
    ...
    try:
        ...
        for e_mode in np.arange(0, len(f_opt_m)):
            ...
            for b in np.arange(0, len(lx)):
                ...
                if ((np.absolute(lx[b]-ax) < imp_x) and (np.absolute(ly[b]-ay) < imp_y) and ((np.absolute(lz[b]-az) < imp_z))):
        ...
        if ((f_opt > 175.28e12) and (f_opt < 203.98e12)):
            ...
            g_sm_val = np.array(model.evaluate(['x', 'y', 'z', '0.5*sqrt((0.3*0.3)* ( 2*3.14159/(sqrt(105e10[Pa]*(1 - 0.1)/(3515[kg/m^3]*(1+0.1)*(1-2*0.1)))/solid.freq) * 2*3.14159/(sqrt(105e10[Pa]*(1 - 0.1)/(3515[kg/m^3]*(1+0.1)*(1-2*0.1)))/solid.freq) ) /(2*3515[kg/m^3]*hbar_const*2*3.14159*solid.freq*int_mech_all(solid.Wk + solid.Ws)/(solid.Wk + solid.Ws)))*8.77*1.6e-19[J]'], unit ='Hz', dataset = '3D mech//Solution 3'))        
            ...
            for index in np.arange(0,len(g_om)):
                ...
                for b in np.arange(0, len(g_sm_val[0][0])):
                    ...
                    if ((np.absolute(x[b]-ax) < g_sm_x) and (np.absolute(y[b]-ay) < g_sm_y) and ((np.absolute(z[b]-az) < g_sm_z)) and ~np.isnan(g_sm[index][b])):
            ...
            if ((f_mech > 3.0e9) and (f_mech < 7.37e9)):
                ...
                if (fid > 0.0):
                    ...
                    with open("output_Si_CF.txt", "a") as mf:
